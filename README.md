...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/opencitations_references \
  -H 'Content-Type: application/json' \
  -d '{"doi":"10.1371/journal.pone.0123456"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/opencitations_references`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "opencitations": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-opencitations"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-opencitations
```
